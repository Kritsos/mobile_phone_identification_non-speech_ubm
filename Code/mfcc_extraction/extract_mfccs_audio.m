% -----------------> extract_mfccs_audio.m <------------------- 

% The following code extracts the MFCCs of the train and test speakers for 
% each subset of the augmented dataset. For each phone and for each speaker 
% (for the train and the test set) it saves a 23xnumframes  matrix containing 
% the corresponding MFCCs
    ...
    for n = 1:2
        ...
        for i = 3:numel(d)
            ...
            for j = 1:12
                ...
                % extract MFCCs on 20ms frames with 10ms hop
                y = extract_features(x, fs);
        
                % go back to initial directory to save the MFCCs
                cd("..\..\") 
                filename = "data\MFCCs\audio\" + databases_names{m} + "\" + folder + phones{i - 2} + "\mfccs_" + speakers{j}{1}(1:end-4) + ".mat";
                save(filename, "y");
