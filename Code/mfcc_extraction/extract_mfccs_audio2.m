% -----------------> extract_mfccs_audio2.m <------------------- 

% The following code extracts the MFCCs of the train and test speakers of 
% the ccnu_mobile dataset. For each phone and for each speaker (for the
% train and the test set) it saves a 39xnumframes  matrix containing  the 
% corresponding MFCCs and their deltas and delta-deltas (13 of each)
...
for i = 3:numel(d1)
    ...
    for j = 1:5
        ...
        filename = "data2\MFCCs\audio\" + phones{i - 2} + "\mfccs_speaker" + num2str(j) + ".mat";
        save(filename, "y");
