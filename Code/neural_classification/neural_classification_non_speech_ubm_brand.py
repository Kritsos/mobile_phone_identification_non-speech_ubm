...
NUM_CLASSES = 9  # Number of categories of classification, number of output units of fc layer
...
def scheduler(epoch):
    # The learning rate decreases to 1/10 of the original rate per 20 epochs.
...
def build_model():
    ...
    hidden9 = keras.layers.Dense(9, activation='softmax', name='layers_softmax1')(hidden4)

    hidden50 = keras.layers.Dense(9, activation='softmax', name='layers_softmax2')(hidden16)
    ...
    hidden100 = keras.layers.Reshape((2, 1024, 1))(hidden18)

    hidden19 = keras.layers.Conv2D(16, (3, 3), strides=(1, 2),activation='relu', padding='same', data_format='channels_last', name='layer2_con8')(hidden100)
    ...
    hidden24 = keras.layers.Reshape((2, 1, 1))(hidden23)
    ...
    hidden32 = Dense(9, activation='softmax', name='fc3')(hidden31)
...
if __name__ == '__main__':
    ...
    x_train_mfcc1 = scipy.io.loadmat("x_train_mfcc_non_speech.mat")
    x_train_mfcc1 = x_train_mfcc1["x_train_mfcc"]
    x_train_gsv = scipy.io.loadmat("x_train_gsv_ubm_adapted.mat")
    x_train_gsv = x_train_gsv["x_train_gsv"]
    y_train = scipy.io.loadmat("y_train_brand.mat")
    y_train = y_train["y_train_brand"]

    x_test_mfcc1 = scipy.io.loadmat("x_test_mfcc_non_speech.mat")
    x_test_mfcc1 = x_test_mfcc1["x_test_mfcc"]
    x_test_gsv = scipy.io.loadmat("x_test_gsv_ubm_adapted.mat")
    x_test_gsv = x_test_gsv["x_test_gsv"]
    y_test = scipy.io.loadmat("y_test_brand.mat")
    y_test = y_test["y_test_brand"]

    reduce_lr = LearningRateScheduler(scheduler)
    m.fit([x_train_mfcc1,x_train_gsv], [y_train,y_train,y_train], epochs=100, batch_size=16, callbacks=[reduce_lr])
    #m.save('m_37.h5')
    accuracy = m.evaluate([x_test_mfcc1,x_test_gsv], [y_test,y_test,y_test])
    #print('loss:', loss)
    print(m.metrics_names)
    print('accuracy:', accuracy) # the last item in the list is the fusion branch classification accuracy

    pred = m.predict([x_test_mfcc1,x_test_gsv])
    pred = pred[2] # keep only the fusion branch output
    # convert to list of predicted integers
    pred = np.argmax(pred, 1) + 1 # + 1 is needed because the counting should start from 1 not 0
    ground_truth = np.argmax(y_test, 1) + 1

    brands = ["APPLE", "HONOR", "HUAWEI", "NUBIA", "OPPO", "SAMSUNG", "VIVO", "XIAOMI", "ZTE"]

    C = confusion_matrix(ground_truth, pred)
    disp = ConfusionMatrixDisplay(C, display_labels=brands)
    disp.plot()
    plt.show()