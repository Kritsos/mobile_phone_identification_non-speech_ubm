% -----------------> ML_classifier_non_speech_model_pitch.m <------------------- 

% The following code uses the model GMMs trained on the non speech MFCCs of
% the baseline + pitch dataset and Maximum Likelihood classification to classify 
...
for i = 1:length(num_comp)
    ...
    acc_pitch = sum(ground_truth(subset == 2) == pred(subset == 2)) / 252;

    disp("Accuracy of " + num2str(num_comp(i)) + "-component GMM: " + num2str(100 * acc) + "%: " + ...
         "Baseline: " + num2str(100 * acc_baseline) + "% / " + "Pitch: " + num2str(100 * acc_pitch) + "%")
