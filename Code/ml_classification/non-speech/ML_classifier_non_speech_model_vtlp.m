% -----------------> ML_classifier_non_speech_model_vtlp.m <------------------- 

% The following code uses the model GMMs trained on the non speech MFCCs of
% the baseline + vtlp dataset and Maximum Likelihood classification to classify 
...
% example comes (1: baseline, 2: vtlp)
...
for i = 1:length(num_comp)
    ...
    acc_vtlp = sum(ground_truth(subset == 2) == pred(subset == 2)) / 252;

    disp("Accuracy of " + num2str(num_comp(i)) + "-component GMM: " + num2str(100 * acc) + "%: " + ...
         "Baseline: " + num2str(100 * acc_baseline) + "% / " + "VTLP: " + num2str(100 * acc_vtlp) + "%")
