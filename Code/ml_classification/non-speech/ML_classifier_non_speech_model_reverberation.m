% -----------------> ML_classifier_non_speech_model_reverberation.m <------------------- 

% The following code uses the model GMMs trained on the non speech MFCCs of
% the baseline + reverberation dataset and Maximum Likelihood classification to classify 
% each recording into one of the 21 models. The classification is done using
% the negative log likelihood, that is each recording is classified into 
% the model whose GMM gives the smallest negative log likelihood value 
...
% example comes (1: baseline, 2: reverberation)
...
for i = 1:length(num_comp)
    ...
    acc_reverberation = sum(ground_truth(subset == 2) == pred(subset == 2)) / 252;

    disp("Accuracy of " + num2str(num_comp(i)) + "-component GMM: " + num2str(100 * acc) + "%: " + ...
         "Baseline: " + num2str(100 * acc_baseline) + "% / " + "Reverberation: " + num2str(100 * acc_reverberation) + "%")

    figure(i)
    C = confusionmat(ground_truth, pred);
    confusionchart(C, phones);
    title(num2str("Model identification on non speech frames with a " + num_comp(i)) + "-component GMM")
end
