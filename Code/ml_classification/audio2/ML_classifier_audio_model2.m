...
% negative log likelihood, that is each recording is classified into the model 
% whose GMM gives the smallest negative log likelihood value on the 
% recording's MFCCs
...
for i = 1:45
    ...
    % for each speaker
    for j = 1:3
        % skip the first two folders (. and ..) and the first two train
        % speakers
...
title("Model identification on audio frames with a 64-component GMM")
