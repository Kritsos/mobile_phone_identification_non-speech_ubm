% -----------------> train_gmms_audio_speakers.m <------------------- 

% The following code uses the extracted training and testing audio MFCCs of 
% all the subsets of the dataset and uses them to train speaker dependent
...
for n = 1:2
    ...
    for m = 1:length(databases_names)
        ...
        for i = 1:length(phones)
            ...
            folder = "data\MFCCs\audio\" + databases_names{m} + "\" + f + phones{i} + "\";
            ...
            for j = 1:length(speakers)
                ...
                for k = 1:length(num_comp)
                    ...
                    filename = "data\GMMs\audio\speakers\" + num2str(num_comp(k)) + "\" + phones{i} + "\gmm_speaker_" + num2str(speakers(j)) ...
