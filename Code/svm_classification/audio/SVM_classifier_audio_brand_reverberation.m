% -----------------> SVM_classifier_audio_brand_reverberation.m <------------------- 

% The following code uses the multiclass SVMs trained on the MFCCs of all
% the frames of the baseline + reverberation dataset to classify each test 
...
for i = 1:length(num_comp)
    ...
    title(num2str("Brand identification on audio frames with a " + num_comp(i)) + "-component means GSV SVM")
    ...
    C = confusionmat(y, pred);
    confusionchart(C, brands);
    title(num2str("Brand identification on audio frames with a " + num_comp(i)) + "-component means and covariance GSV SVM")
end
