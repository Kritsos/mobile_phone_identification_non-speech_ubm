% -----------------> SVM_classifier_non_speech_model_reverberation.m <------------------- 

% The following code uses the multiclass SVMs trained on the MFCCs of the
% non speech frames of the baseline + reverberation dataset to classify each test 
% speaker recording to one of the 21 phone models
...
for i = 1:length(num_comp)
    ...
    acc_reverberation = sum(y(subset == 2) == pred(subset == 2)) / 252;
    disp("Accuracy of " + num2str(num_comp(i)) + "-component means GSV SVM: " + num2str(100 * acc) + "%")
    disp("Baseline: " + num2str(100 * acc_baseline) + "% / " + "Reverberation: " + num2str(100 * acc_reverberation) + "%")

    figure(2 * (i - 1) + 1)
    C = confusionmat(y, pred);
    confusionchart(C, phones);
    title(num2str("Model identification on non speech frames with a " + num_comp(i)) + "-component means GSV SVM")
    ...
    C = confusionmat(y, pred);
    confusionchart(C, phones);
    title(num2str("Model identification on non speech frames with a " + num_comp(i)) + "-component means and covariance GSV SVM")
end
