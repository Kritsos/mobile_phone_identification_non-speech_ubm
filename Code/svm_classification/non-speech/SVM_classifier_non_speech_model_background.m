% -----------------> SVM_classifier_non_speech_model_background.m <------------------- 

% The following code uses the multiclass SVMs trained on the MFCCs of the
% non speech frames of the baseline + background dataset to classify each test 
...
for i = 1:length(num_comp)
    ...
    acc_background = sum(y(subset == 2) == pred(subset == 2)) / 252;
    disp("Accuracy of " + num2str(num_comp(i)) + "-component means GSV SVM: " + num2str(100 * acc) + "%")
    disp("Baseline: " + num2str(100 * acc_baseline) + "% / " + "Background: " + num2str(100 * acc_background) + "%")
