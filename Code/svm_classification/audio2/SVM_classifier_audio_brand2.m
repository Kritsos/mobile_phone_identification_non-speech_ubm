...
% recording to one of the 9 brands
...
for j = 1:length(phones)
    % folder of phone's num_comp-component speaker GMMs
...
title(num2str("Brand identification on audio frames with a " + num_comp) + "-component means GSV SVM")
...
C = confusionmat(y, pred);
confusionchart(C, brands);
title(num2str("Brand identification on audio frames with a " + num_comp) + "-component means and covariance GSV SVM")

% return to script directory
cd("svm_classification\audio2")