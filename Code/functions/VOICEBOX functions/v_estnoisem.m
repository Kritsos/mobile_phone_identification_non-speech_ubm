...
% convergence of the algorithm. A minor improvement was reported in [3] but
...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    if nv<4            % algorithm doesn't work for minuscule frames
        nv=4;
    ...
    qeqimin=1/qeqmax; % minimum value of Qeq per frame inverse
    ...
    if isempty(yf)      % provide dummy initialization
        ...
        if ~nrcum       % initialize values for the first frame
            ...
            actmin=repmat(Inf,1,nrf);   % Running minimum estimate
            actminsub=actmin;           % sub-window minimum estimate
            subwc=nv;                   % force a buffer switch on the first loop
            actbuf=repmat(Inf,nu,nrf);  % buffer to store subwindow minima
            ibuf=0;
            lminflag=zeros(1,nrf);      % flag to remember local minimum
        end
        ...
        for t=1:nr              % we use t instead of lambda in the paper
            ...
            if subwc>1 && subwc<nv              % middle of the buffer - allow a local minimum
                lminflag=lminflag | kmod;        % potential local minimum frequency bins
                pminu=min(actminsub,pminu);
                sn2=pminu;
            else
                if subwc>=nv                    % end of the buffer - do a buffer switch
            ...
            x(t,:)=sn2;
            qisq=sqrt(qeqi);
            % empirical formula for the standard error based on Fig 15 of [2]
            xs(t,:)=sn2.*sqrt(0.266*(nd+100*qisq).*qisq/(1+0.005*nd+6/nd)./(0.5*qeqi.^(-1)+nd-1));
        end
    end
    if nargout>1    % we need to store the state for the next time
        zo.nrcum=nrcum;
        zo.p=p;
        zo.ac=ac;
        zo.sn2=sn2;
