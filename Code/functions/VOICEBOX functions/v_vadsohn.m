...
%   pp.qith     % Q-inverse thresholds to select the maximum noise slope [0.03 0.05 0.06 Inf]
...
nf = ni * no; % fft length
%nd = floor(ni * (no - 1) / 2); % output delay relative to the start of the frame
nd = 0; % no delay
w = hamming(nf + 1)'; w(end) = []; % for now, always use the hamming window
w = w / sqrt(sum(w(1:ni:nf) .^ 2)); % normalize to give an overall gain of 1
ns = length(s);
y = v_enframe(s, w, ni);
yf = v_rfft(y, nf, 2);
